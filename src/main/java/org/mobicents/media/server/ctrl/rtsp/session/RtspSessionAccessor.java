...
public interface RtspSessionAccessor {
  ...
  /**
   * 使用指定sessionKey获取一个session.如果无法使用指定sessionKey获取到session, 且<code>create</code>为<code>true</code>
   * ,则一个新session被创建,并绑定到对 应的请求中,否则放回null.
   * 
   * @param create 如果指定sessionkey的session不存在,且该参数为true则创建一个session.
   * 
   * @return 分配或创建的session.如果指定sessionKey的session不存在, 且<code>create</code>为<code>false</code>
   *         则返回</code>null</code>.
   */
